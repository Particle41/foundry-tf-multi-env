/*
  ...
  Terraform backend resources [bucket, table, key] deployed from a
...
data "aws_ssm_parameter" "tfstate_table" {
  name = "/infra/tfstate/table"
}
...
data "aws_dynamodb_table" "tfstate" {
  name = data.aws_ssm_parameter.tfstate_table.value
}
...
resource "aws_iam_role_policy_attachment" "github_actions_codeartifact_read" {
  role       = aws_iam_role.github_actions.id
